...
objective = (y-ref)'*wQ*(y-ref)+(u)'*wR*(u);
constraints = [];
for k = 1:N
    constraints = [constraints, x(1,k+1) == x(1,k) + Ts*x(2,k)];
    constraints = [constraints, x(2,k+1) == x(2,k) + Ts*(mu*(1-x(1,k)^2)*x(2,k)-x(1,k)+(u(k)))];
    constraints = [constraints, y(k,1) == x(1,k)];
end 
for k = 1:N
...
savefig('product_kernel_control_NMPC.fig') %change it with the name you want to give to the .fig plot
print -depsc product_kernel_control_NMPC %change it with the name you want to give to the .eps figure


save('plot_data_NMPC','r_plot_NMPC', 'yk_NMPC','uk_NMPC') 