...
[~,Sphiy,Vphiy] = svd([PHI;Y],'vector');
PHIY_reduced = [PHI;Y]*Vphiy(:,1:Ir);
...
objective = (y-ref)'*wQ*(y-ref)+(u)'*wR*(u) + lambda*g'*g;
% 
...
constraints = [ PHIY_reduced*g == [zeros(q,1); (y - Theta*Phi(u,x0,U,X0,sigma_u,sigma_x,K_mean_col,K_mean_all,Vk(:,1:q)) ) ]];
...
save('plot_data_SVD','r_plot_SVD', 'yk_SVD','uk_SVD') 